    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for strategy in ["bm25", "vector", "hybrid", "adaptive"]:
            ...
            for q in dataset:
                ...
                if q["answerable"]:
                    recall = len(ids & set(q["relevant_document_ids"])) / len(
                        q["relevant_document_ids"]
                    )
                    recalls.append(recall)
                    answers.append(
                        not response["refused"]
                        and q["answer_contains"] in response["answer"]
                    )
                ...
                details.append(
                    {
                        ...
                        "retrieved_document_ids": sorted(ids),
            ...
            results.append(
                {
                    ...
                    "answer_keyword_accuracy": avg(answers),
                    "false_refusal_rate": avg(false_refusals),
                    "correct_refusal_rate": avg(correct_refusals),
        ...
        report = {
            ...
            "scope": "自建小样本，4 条开发题与 28 条固定测试题；文档级 Recall@5。关键词检查不等于人工准确率，不评估真实 LLM 幻觉率。",
            "test_count": len(dataset),
            "answerable_count": sum(q["answerable"] for q in dataset),
            "embedding": service.embedder.fingerprint,
            "generation": os.getenv("RAG_GENERATION", "extractive"),
            "results": results,
        }
    output_name = (
        "evaluation-semantic.json"
        if os.getenv("RAG_EMBEDDING", "hash") != "hash"
        ...
        json.dumps(
            {
                ...
                "results": [
                    {k: v for k, v in row.items() if k != "details"} for row in results
                ],
            },
            ensure_ascii=False,
            indent=2,
