...
class RagService:
    ...
    def seed(self):
        ...
        for doc in json.loads(path.read_text(encoding="utf-8")):
            self.store.upsert(doc["id"], doc["title"] + ".md", doc["content"].encode(), doc["department"], doc["version"])

    def search(self, query, strategy="adaptive", department=None, version=None, k=5, rerank=False):
        if strategy not in {"adaptive", "bm25", "vector", "hybrid"}:
            raise ValueError("未知检索策略")
        route, reason = query_route(query) if strategy == "adaptive" else (strategy, "手动选择用于对照实验")
        ...
        dense = cosine(self.embedder.embed([query], query=True)[0], [json.loads(row["vector"]) for row in rows]) if rows and route != "bm25" else [0.0] * len(rows)
        ...
        if rerank and indices:
            ...
            if not key:
                ...
                try:
                    response = httpx.post("https://api.cohere.com/v2/rerank", headers={"Authorization": f"Bearer {key}"}, json={"model": os.getenv("COHERE_MODEL", "rerank-multilingual-v3.0"), "query": query, "documents": [texts[i] for i in indices], "top_n": k}, timeout=15)
                    response.raise_for_status()
                    indices = [indices[item["index"]] for item in response.json()["results"]]
                except (httpx.HTTPError, KeyError, IndexError, TypeError):
                    warning = "精排服务失败；已降级到原始召回排名"
        query_terms = set(tokens(query))
        hits = []
        for index in indices[:k]:
            row = {key: value for key, value in rows[index].items() if key not in {"vector", "embedding_model"}}
            coverage = len(query_terms & set(tokens(texts[index]))) / max(len(query_terms), 1)
            hits.append({**row, "score": round(scores.get(index, 0), 6), "coverage": round(coverage, 4)})
        return {"hits": hits, "route": route, "route_reason": reason, "embedding": self.embedder.fingerprint, "warning": warning}

    def ask(self, query, strategy="adaptive", department=None, version=None, k=5, rerank=False, history=None):
        ...
        if usable and mode == "openai":
            ...
            claims = [{"text": hit["text"], "citation_id": hit["id"], "quote": hit["text"]} for hit in usable[:2]]
        answer = "现有知识库证据不足，无法可靠回答。请补充问题细节、调整版本筛选或导入相关文档。" if refusal else "\n\n".join(f"{claim['text']} [{claim['citation_id']}]" for claim in claims)
        response = {**result, "query": original, "effective_query": query, "answer": answer, "claims": claims, "refused": refusal, "generation": mode, "threshold": threshold, "trace_id": uuid.uuid4().hex, "latency_ms": round((time.perf_counter() - start) * 1000, 2)}
        ...
        with (self.directory / "traces.jsonl").open("a", encoding="utf-8") as stream:
            stream.write(json.dumps({key: response[key] for key in ["trace_id", "route", "embedding", "generation", "refused", "latency_ms"]}) + "\n")
        return response
    ...
    def generate(self, query, hits):
        ...
        response = httpx.post(os.getenv("MODEL_BASE_URL", "http://127.0.0.1:11434/v1").rstrip("/") + "/chat/completions", headers={"Authorization": "Bearer " + os.getenv("MODEL_API_KEY", "")}, json={"model": os.getenv("MODEL_NAME", "qwen3:8b"), "messages": [{"role": "system", "content": prompt}, {"role": "user", "content": json.dumps({"question": query, "evidence": [{"id": h["id"], "text": h["text"]} for h in hits]}, ensure_ascii=False)}], "temperature": 0, "response_format": {"type": "json_object"}}, timeout=60)
        response.raise_for_status()
        try:
            claims = json.loads(response.json()["choices"][0]["message"]["content"])["claims"]
            evidence = {hit["id"]: hit["text"] for hit in hits}
            if not isinstance(claims, list) or len(claims) > 8:
                return []
            for claim in claims:
                if not isinstance(claim, dict) or not all(isinstance(claim.get(key), str) and claim[key].strip() for key in ["text", "citation_id", "quote"]):
                    return []
                if claim["citation_id"] not in evidence or claim["quote"] not in evidence[claim["citation_id"]]:
                    return []
            return claims
